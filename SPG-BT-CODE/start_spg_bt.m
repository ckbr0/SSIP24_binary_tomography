...
%  matlabpool; % have to be activate to reach full speed 
   ...
   %%%%%% START: CALCULATE PROJECTION DATA %%%%%%%%%%%%%%%%%%
     ...
     %  angles_deg=[0 90]; proj_directions='45'; [M c]=proj_data_calc(ph_orig,angles_deg);
...
img = imread("../phantoms/butterfly_dart_64.pgm");
disp(size(img));
%img = padarray(img,[3,3],0,"post");
img(img > 0) = 1;
%imshow(img)
disp(size(img));
img = reshape(img.',1,[]);
...
load("../projMat/sysMat_na36_px64_ds4_dt26.mat");
M = G;
%M = transpose(M);
disp(size(img'));
disp(size(M));
...
if display_progress,
    ...
    title(['ORIGINAL ori: ' num2str(orientation_vec(ph_orig(:))) ' CM: ' num2str(convexity_measure(ph_orig)) ]);
...
if display_progress,   
     ...
     title(['RECONSTRUCTION ori: ' num2str(orientation_vec(Xnew)) ' CM: ' num2str(convexity_measure(rowreshape(Xnew,n,n)')) ' k=' num2str(0)  ]);
     drawnow; 
end;
...
while (total_iterations<50) || ( max(min(Xnew,1-Xnew))>Eout && norm(Xold-Xnew,1)>0 && total_iterations<max_tot_it), % GENERAL STOPPING CRITERIUM
    ...
    if display_progress,   
     ...
     title(['RECONSTRUCTION ori: ' num2str(orientation_vec(Xnew)) ' CM: ' num2str(convexity_measure(rowreshape(Xnew,n,n)')) ' k=' num2str(k-1) ' it=' num2str(total_iterations) ]);
     drawnow; 
...
title(['RECONSTRUCTION orientation.:' num2str(orient_rec) ' CM: ' num2str(convexity_measure(out_im)) '  it=' num2str(total_iterations) ]);
drawnow;
